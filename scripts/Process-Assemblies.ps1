...
Write-ColoredOutput "City of Gangsters Assembly Processor" $Cyan
...
if ([string]::IsNullOrEmpty($GameDir)) {
    $possiblePaths = @(
        ...
        "C:\Program Files\Steam\steamapps\common\City of Gangsters",
        ".\GameDir"
    )
...
$ManagedDir = Join-Path $GameDir "CoG_Data\Managed"
if (!(Test-Path $ManagedDir)) {
    Write-ColoredOutput "[ERROR] Managed directory not found: $ManagedDir" $Red
    Write-ColoredOutput "[HINT] Expected: GameDir\CoG_Data\Managed\" $Yellow
    exit 1
}
...
Auto-generated by Process-Assemblies.ps1"
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Output "   - Check: https://github.com/MrPurple6411/CityofGansters/actions"
    } else {
